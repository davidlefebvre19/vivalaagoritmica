...
public class TwoThreeTree {
    ...
    public static class Node23 {
        ...
        int size;                    // taille du sous-arbre (somme des tailles + 1)

        public Node23(List<String> key, List<Node23> children) {
            this.key = key;
    ...
    public Node23 preorderTo23Tree(List<Object> preorder) {
        // index global passé par référence
        int[] index = {0};
    ...
    private Node23 build(List<Object> preorder, int[] index, String min, String max) {
        ...
        // Vérifie si la clé respecte les bornes
        String firstKey = keys.get(0);
        if (min != null && firstKey.compareTo(min) <= 0) return null; // first key smaller than min
        String lastKey = keys.get(keys.size() - 1);
        if (max != null && lastKey.compareTo(max) >= 0) return null; // second key gretaer than max
        ...
        return new Node23(keys, children);
    }
    ...
    public static void main(String[] args) {
        TwoThreeTree treeBuilder = new TwoThreeTree();
